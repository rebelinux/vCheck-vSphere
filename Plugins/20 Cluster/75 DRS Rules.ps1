...
ForEach ($DRSCluster in $Clusters) {
    ...
    ForEach ($myDRSRule in $myDRSRules) {
        ...
        } elseif (($myDRSRuleType -ne "VMHostAffinity") -and (($myDRSRule.VMIDS).Count -gt 1)) {
            #### else, if a different type of rule (VMAffinity,VMAntiAffinity) and more than one VM is defined
